          ...
          <header className='flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4 mb-10 border-b border-border/50 pb-6'>
            <div className='space-y-1.5'>
              ...
              <p className='text-muted-foreground font-medium text-sm'>
                {meta.title} <span className="mx-2 opacity-50">|</span> {meta.courseName}
              </p>
            </div>
              ...
              <CardContent className='p-6 sm:p-8 space-y-6'>
                ...
                <div className='space-y-5'>
                  <div>
                    ...
                    <Input {...form.register('title')} className='bg-muted/10 border-border/50 focus-visible:ring-primary/20 h-11 rounded-xl' />
                  </div>
                  <div>
                    <label className='text-[11px] font-bold uppercase tracking-wider text-muted-foreground mb-2 block'>Mô tả ngắn</label>
                    <Input {...form.register('shortDescription')} className='bg-muted/10 border-border/50 focus-visible:ring-primary/20 h-11 rounded-xl' />
                  </div>
                  <div>
                    <label className='text-[11px] font-bold uppercase tracking-wider text-muted-foreground mb-2 block'>
                      Mô tả chi tiết (Ngữ cảnh AI)
                    </label>
                    <Textarea {...form.register('description')} className='bg-muted/10 border-border/50 focus-visible:ring-primary/20 min-h-[100px] rounded-xl' />
                  </div>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent className='p-6 sm:p-8'>
                ...
                <div className='flex flex-col sm:flex-row gap-6 sm:items-end'>
                  ...
                  <div className='flex-1 grid grid-cols-1 sm:grid-cols-2 gap-4 w-full'>
                    <label htmlFor='cert' className='flex items-center gap-3 border border-border/50 rounded-xl p-4 bg-muted/10 hover:bg-muted/30 transition-colors cursor-pointer group'>
                      <Controller
                        control={form.control}
                        name='issueCertificate'
                        render={({ field }) => (
                          <Checkbox id='cert' checked={field.value} onCheckedChange={field.onChange} className='h-5 w-5 rounded-md data-[state=checked]:bg-primary data-[state=checked]:border-primary shadow-sm' />
                        )}
                      />
                    ...
                    <label htmlFor='unlock' className='flex items-center gap-3 border border-border/50 rounded-xl p-4 bg-muted/10 hover:bg-muted/30 transition-colors cursor-pointer group'>
                      <Controller
                        control={form.control}
                        name='unlockNextCourse'
                        render={({ field }) => (
                          <Checkbox id='unlock' checked={field.value} onCheckedChange={field.onChange} className='h-5 w-5 rounded-md data-[state=checked]:bg-primary data-[state=checked]:border-primary shadow-sm' />
                        )}
                      />
                      <span className='text-sm font-semibold select-none group-hover:text-primary transition-colors'>
            ...
            <div className='sticky bottom-0 -mx-6 md:-mx-10 lg:-mx-12 -mb-6 md:-mb-10 lg:-mb-12 mt-12 p-4 sm:p-6 bg-gradient-to-t from-background/95 to-transparent z-20 pointer-events-none'>
              <div className='max-w-3xl mx-auto flex flex-col sm:flex-row justify-between sm:items-center gap-4 border border-primary/50 bg-background/80 backdrop-blur-xl shadow-xl rounded-2xl p-4 sm:px-6 pointer-events-auto'>
                <div>
                  ...
                  <p className='text-sm text-muted-foreground font-medium'>Đảm bảo mỗi câu hỏi có ít nhất một đáp án đúng.</p>
                </div>
