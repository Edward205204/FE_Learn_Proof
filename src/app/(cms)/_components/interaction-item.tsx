...
export function InteractionItem({ data }: InteractionItemProps) {
  return (
    <Card className='border-none shadow-sm relative overflow-hidden'>
      <div
        className={`absolute left-0 top-0 bottom-0 w-1 ${data.type === 'discussion' ? 'bg-orange-400' : 'bg-primary'}`}
      />
        ...
        <div className='flex justify-between items-start mb-4'>
          <div className='flex gap-4'>
            ...
            <div>
              ...
              <p className='text-xs text-muted-foreground'>{data.user.courseName} · 2 giờ trước</p>
            </div>
          </div>
