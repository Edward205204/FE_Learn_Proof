...
import { AlertCircle, BookOpen, Plus, RefreshCw, type LucideIcon } from 'lucide-react'
...
import { STATUS_LABEL } from '../_constants/course-manager'
...
type FixedCourseStatus = Exclude<ManagerFilterStatus, 'ALL'>

type CourseManagementPageContentProps = {
  fixedStatus?: FixedCourseStatus
  title?: string
  description?: string
  emptyTitle?: string
...
export function CourseManagementPageContent({
  fixedStatus,
  title = 'Quản lý Khóa học',
  description = 'Tạo, cập nhật và quản lý tất cả các khóa học của bạn tại đây.',
  emptyTitle = 'Chưa có khóa học nào',
  emptyDescription = 'Bắt đầu bằng cách tạo khóa học đầu tiên của bạn.',
  icon: Icon = BookOpen
}: CourseManagementPageContentProps) {
  const [queryParams, setQueryParams] = useQueryParams(GetMyCoursesManagerQuerySchema)
  const effectiveQueryParams = useMemo(
    () => ({
      ...queryParams,
      status: fixedStatus ?? queryParams.status
    }),
    [fixedStatus, queryParams]
  )

  const { data, isLoading, isFetching, isError, error, refetch } = useGetMyCoursesManagerQuery(effectiveQueryParams)
  const prefetchCourseDetail = usePrefetchManagerCourseDetail()

  const courses = data?.items ?? []
  const meta = data?.meta
  const showFilter = !fixedStatus
  const fixedStatusLabel = fixedStatus ? STATUS_LABEL[fixedStatus] : null
    ...
    <div className='space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500'>
        ...
        <div className='space-y-1.5'>
          <div className='flex items-center gap-2 flex-wrap'>
            ...
            {fixedStatus && (
              <Badge
                variant='outline'
                className='rounded-full border-primary/20 bg-primary/5 text-primary text-xs font-semibold'
              >
                {STATUS_LABEL[fixedStatus]}
              </Badge>
            )}
          </div>
      ...
      <div className='flex items-center justify-between bg-muted/30 p-2 sm:p-3 rounded-xl border border-border/50'>
        <div className='flex-1'>
          {showFilter ? (
            <CourseManagerFilter status={effectiveQueryParams.status} onStatusChange={handleStatusChange} />
          ) : (
            <p className='px-2 text-sm font-medium text-muted-foreground'>
              Chỉ hiển thị các khóa học ở trạng thái {fixedStatusLabel}.
            </p>
          )}
        </div>
        {meta && (
      ...
      <div className={`transition-opacity duration-150 ${isFetching ? 'opacity-60' : 'opacity-100'}`}>
        {isLoading ? (
          ...
          <div className='flex flex-col items-center justify-center py-24 text-center'>
            ...
            <p className='text-muted-foreground mt-1 mb-6'>{emptyDescription}</p>
            <Button asChild>
              <Link href={PATH.COURSE_NEW_STEP1}>
