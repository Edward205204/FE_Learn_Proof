...
export default function SearchInput({ className = '', placeholder = 'Tìm kiếm khóa học thực chiến...' }: SearchInputProps) {
  ...
  return (
    <div className={`relative ${className}`} ref={searchRef} onKeyDown={handleKeyDown}>
      ...
      {isSuggestionsOpen && searchQuery.length >= 2 && (
        <div className='absolute top-full left-0 mt-3 w-full md:w-[420px] 
                ...
                {suggestions.map((course, index) => (
                  ...
                  >
                    <div className='relative h-14 w-24 flex-shrink-0 overflow-hidden rounded-lg shadow-sm group-hover:shadow-md transition-shadow'>
                      <Image
                        fill
                        src={course.thumbnail || 'https://images.unsplash.com/photo-1498050108023-c5249f4df085?auto=format&fit=crop&q=80'}
                        alt={course.title}
